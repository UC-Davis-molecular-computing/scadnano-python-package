...
.PARAMETER Issue
    The issue number, e.g. 323.

.PARAMETER Base
    Branch to create from. Defaults to dev; you should not normally change it.

.EXAMPLE
    .\branch-for-issue.ps1 323

    Creates e.g. `323-replace-automated-release-action` from dev and checks it out.
#>
[CmdletBinding()]
param(
    [Parameter(Mandatory = $true, Position = 0, HelpMessage = 'GitHub issue number, e.g. 323')]
...
if (-not (Test-Path (Join-Path $PSScriptRoot '.git'))) {
    Write-Error "Run this script from inside the scadnano-python-package working copy."
    exit 1
}
