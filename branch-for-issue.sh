...
#
# Usage:  ./branch-for-issue.sh 323 [base-branch]
# Example: ./branch-for-issue.sh 323
#          -> creates e.g. 323-replace-automated-release-action from dev
...
if ! git rev-parse --git-dir > /dev/null 2>&1; then
  echo "error: run this script from inside the scadnano-python-package working copy." >&2
  exit 1
fi
